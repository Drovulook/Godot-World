...
namespace godot {
    ...
    void FaceMesh::generate_mesh() {
        ...
        Vector3 pos_center = Vector3(center_x - 0.5f, center_y - 0.5f, 0.5f);

        switch (m_direction) {
            case FRONT:   // +Z
                pos_center = Vector3(pos_center.x, pos_center.y, 0.5f);
                break;
            case BACK:    // -Z
                pos_center = Vector3(-pos_center.x, pos_center.y, -0.5f);
                break;
            case LEFT:    // -X
                pos_center = Vector3(-0.5f, pos_center.y, -pos_center.x);
                break;
            case RIGHT:   // +X
                pos_center = Vector3(0.5f, pos_center.y, pos_center.x);
                break;
            case TOP:     // +Y
                pos_center = Vector3(pos_center.x, 0.5f, -pos_center.y);
                break;
            case BOTTOM:  // -Y
                pos_center = Vector3(pos_center.x, -0.5f, pos_center.y);
                break;
        ...
        Vector3 norm_pos_center = pos_center.normalized(); 
        float glob_u_center = 1.0f - (0.5f + (atan2(norm_pos_center.z, norm_pos_center.x) / (2.0f * Math_PI)));
        float glob_v_center = 0.5f - (asin(norm_pos_center.y) / Math_PI);

        tile_x = ((int)(glob_u_center * 8)) % 8;
        tile_y = 3 - (((int)(glob_v_center * 4)) % 4);
