...
ProvincesManager::ProvincesManager(Ref<Texture2D> city
_texture,
                                   DebugUI *debug_ui)
    : m_city
_image(city
_texture->get_image()), m_debug_ui(debug_ui) {}
...
void ProvincesManager::load_city
_mapping() {
  if (m_city
_data_loaded) {
    return;
  ...
  String json_path =
      "res://assets/city
s/Province_Index_Map_Generated_mapping.json";
  Ref<FileAccess> file = FileAccess::open(json_path, FileAccess::READ);

  if (!file.is_valid()) {
    UtilityFunctions::print("Failed to load city
 mapping file: ",
                            json_path);
    return;
  }
...
s[key];
    ...
    // Vérifier si la city
 a des données (pas juste un objet vide)
    if (city
_info.has("country_code")) {
      ProvinceData city
_data;
      city
_data.country_code = String(city
_info["country_code"]);
      city
_data.country_name = String(city
_info["country_name"]);
      city
_data.city
_name = String(city
...
_data;
  ...
  m_city
_data_loaded = true;
  UtilityFunctions::print("Loaded ", int(m_city
_mapping.size()),
                          " city
s from .json file: ", json_path);
}
...
void ProvincesManager::_input(const Ref<InputEvent> &event) {
  ...
  if (mouse_event.is_valid() && mouse_event->is_pressed() &&
    ...
    //                         mouse_pos.y);
    int city
_id = get_country_at_screen_position(mouse_pos);
...
int ProvincesManager::get_country_at_screen_position(Vector2 screen_pos) {
  ...
  // Obtenir l'ID de la city
 à cette position UV
  int city
_id = get_country_id_at_position(uv_coords);
...
_image.is_valid()) {
  ...
  int img_width = m_city
_image->get_width();
  int img_height = m_city
_image->get_height();
  ...
  Color pixel_color = m_city
_image->get_pixel(pixel_x, pixel_y);

  // Convertir la couleur en ID de city
 selon votre format
...
_id = (r << 16) | (g << 8) | b;
  ...
  // int city
_id = b; // Si l'ID est juste dans le canal bleu

  return city
_id;
}

ProvinceData ProvincesManager::get_city
_from_id(int city
_id) {
  auto it = m_city
_mapping.find(city
