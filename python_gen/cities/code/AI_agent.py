...
class AiAgent:
    ...
    def create_prompt(self, value_dict):
        ...
        {{
            ...
            "description": "2-3 sentence description of the city at that time using the provided description and your knowledge if you can provide any relevant information"
        }}

        IMPORTANT RULES:
        - For "estimated_population": provide ONLY a number (e.g., 15000, not "15,000 inhabitants"). No explanation.
        - For "dominant_culture": use specific civilization names at the time (e.g., "Greek", "Persian", "Egyptian", etc.). Never use "unknown". No explanation. Use short names.
        - For "main_religion": use specific religion names at the time (e.g. "Greek polytheism", "Egyptian religion", "Canaanite religion", "Zoroastrianism", etc.). Only use "unknown" if you have absolutely no historical information. No explanation. Use short names.
    ...
    def parse_ollama_response(self, response):
        try:
            ...
            print("Erreur de décodage JSON dans la réponse d'Ollama.")
    ...
    def enrich_city(self, value_dict):
        print("\n" + "-"* 60,f"\nville en cours de traitement: {value_dict.get('name', 'Unknown City')}")
        ...
        if response:
            ...
            print(f"📍 Nom: {value_dict.get('name', 'Unknown City')}")
    ...
    def enrich_data(self, data):
        ...
        print(f"Traitement de {self.dict_len} villes...")
        for key, value in data.items():
            self.enrich_city(value)
        print(f"Dictionnaire enrichi")
